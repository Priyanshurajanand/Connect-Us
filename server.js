...
const aboutAll = require("./routes/auth/about");
const start = require("./routes/start");
const profile = require("./routes/auth/profile");
const dashboard = require("./routes/auth/dashboard");
const interviewer = require("./routes/auth/interviewer");
...
app.use(cookie({ maxAge: 30 * 24 * 60 * 60 * 1000, keys: ["Priyanshu"] }));
...
app.get('/api/v1/event-details', async(req, res) =>{
    const eResults = await event.find({});
    // console.log(eresults)

    res.status(200).send(eResults)
})
